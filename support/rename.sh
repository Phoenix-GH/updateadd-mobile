...
filesToModify=$(grep -riIl 'reactnativeseed' --exclude='rename.sh' $appRoot/*)
filesToRename=$(find "${appRoot}/ios" "${appRoot}/android" -type f -ipath '*reactnativeseed*')

# replace strings in files
for fileToModify in $filesToModify; do
  sed -i.bak "s/ReactNativeSeed/${newAppName}/g;s/reactnativeseed/${newLowerCaseName}/g" $fileToModify
done
...
for fileToRename in $filesToRename; do
  newName=$(echo $fileToRename | sed "s/ReactNativeSeed/$newAppName/g;s/reactnativeseed/$newLowerCaseName/g")
  mkdir -p $(dirname "$newName") && $mvCmd "$fileToRename" "$newName"
done

# remove leftover empty directories
rmdir -p $(find "$appRoot" -ipath '*reactnativeseed*' -type d) 2>/dev/null
